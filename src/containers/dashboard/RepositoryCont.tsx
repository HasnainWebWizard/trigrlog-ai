...
export default function RepositoryCont({ repos, selectedRepoId, onSelect }: RepositoryContProps) {
    return (
        <div className="relative w-full">
            ...
            */}
            <div className="scrollbar-hide flex w-full items-center gap-3 overflow-x-auto pb-2 sm:flex-wrap sm:overflow-visible sm:pb-0">
                {repos.map((repo) => (
                    <button
                        key={repo.id}
                        onClick={() => onSelect(repo)}
                        className={`flex items-center gap-2 shrink-0 whitespace-nowrap rounded-full px-4 py-1.5 text-xs font-medium transition-all ${
            ...
            <div className="pointer-events-none absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-[#0d1117] to-transparent sm:hidden" />
        </div>
    );
}